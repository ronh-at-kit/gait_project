...
class TransferConvLSTMFlow(nn.Module):
    def __init__(self):
        ...
        self.avialable_device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

        self.conv_net = PretrainConvFlow()
        self.conv_net.load_state_dict(torch.load('/home/ron/PycharmProjects/Gait2019/gait_project/saved_models/PretrainConvFlow/40to70_ts40/40to70_ts40.pt'))
        self.conv_net.eval()
    ...
    def forward(self, x):
        ...
        x = x.permute(1, 0, 2)
        x, _ = self.lstm(x)
        x = x.contiguous().view(TIME_STEPS * BATCH_SIZE, LSTM_HIDDEN_FEATURES)
