...
class ConvLSTMFlowFull(nn.Module):
    def __init__(self):
        super(ConvLSTMFlowFull, self).__init__()
    ...
    def forward(self, x):
        # print("X[0]:",x[0].size())
        x_arr = torch.zeros(TIME_STEPS, BATCH_SIZE, CHANNELS_OUT, IMAGE_AFTER_CONV_SIZE_H, IMAGE_AFTER_CONV_SIZE_W)# .to(self.avialable_device)

        for i in range(TIME_STEPS):
            x_arr[i] = self.features(x[i])
            # print("X_arr[i]", x[i].size())

        x = x_arr.view(TIME_STEPS, BATCH_SIZE, CHANNELS_OUT*IMAGE_AFTER_CONV_SIZE_W*IMAGE_AFTER_CONV_SIZE_H)
        #         print("x.Size()",x.size())
        x = x.permute(1, 0, 2)
        x, _ = self.lstm(x)
        x = self.classifier(x)
